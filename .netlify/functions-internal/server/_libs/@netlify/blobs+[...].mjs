...
//#endregion
//#region node_modules/@netlify/blobs/dist/chunk-6TDSNTDP.js
...
var blobsErrorMessage = (res, context, responseBody) => {
	...
	if (!res.headers.get(NF_ERROR) && responseBody) message += `: ${responseBody}`;
	return message;
};
...
var createBlobsInternalError = async (res, context = {}) => {
	return new BlobsInternalError(res, context, await res.clone().text().catch(() => void 0));
};
...
var fetchAndRetry = async (fetch, url, options, attemptsLeft = MAX_RETRY, getRetryUrl) => {
	try {
		...
		if (attemptsLeft > 0 && isRetryable) {
			...
			return fetchAndRetry(fetch, getRetryUrl ? await getRetryUrl() : url, options, attemptsLeft - 1, getRetryUrl);
		}
		return res;
	} catch (error) {
...
var Client = class {
	...
	async getFinalRequest({ consistency: opConsistency, key, metadata, method, parameters = {}, storeName }) {
		...
		if (res.status !== 200) throw await createBlobsInternalError(res, {
			method,
			storeName
		});
	...
	async makeRequest({ body, conditions = {}, consistency, headers: extraHeaders, key, metadata, method, parameters, storeName }) {
		...
		let getRetryUrl;
		if (usesSignedUrl) getRetryUrl = async () => {
			return (await this.getFinalRequest({
				consistency,
				key,
				metadata,
				method,
