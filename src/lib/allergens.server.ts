...
export type AllergenIndex = Record<string, string[]>;
...
for that item (sometimes labeled "Allergens" or "Allergens & Meat Types").
Return ONLY JSON of the shape:
{"items":[{"name":"Cheese Pizza","allergens":["Wheat","Soy","Milk","Tomato"]}]}
Rules:
...
async function parseItemsWithAI(text: string): Promise<AllergenIndex> {
  ...
  for (const item of parsed.items ?? []) {
    ...
    index[key] = Array.isArray(item.allergens) ? item.allergens.filter(Boolean) : [];
  }
  return index;
}
